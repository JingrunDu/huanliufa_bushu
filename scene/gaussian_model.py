...
class GaussianModel:
    ...
    def densify_and_prune_fastgs(self, max_screen_size, min_opacity, extent, radii, args, importance_score = None, pruning_score = None):
        ...
        if remove_budget:
            ...
            # torch.multinomial supports at most 2^24 categories; fall back to
            # top-k selection when the number of Gaussians exceeds this limit.
            max_multinomial_categories = 2 ** 24
            if n_init_points > max_multinomial_categories:
                _, topk_indices = torch.topk(padded_importance, remove_budget)
